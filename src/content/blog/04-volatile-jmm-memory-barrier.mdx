...
补充一个常见细节：在现代 64 位 HotSpot 虚拟机及常见 64 位 CPU 架构（如 x86-64、AArch64）下，普通 `long`、`double` 的读写在实现层面已基本保证单次原子性；但在 Java 语言规范（JLS §17.7）中，对非 volatile 的 64 位值写入仍允许拆分为两次 32 位操作。普通读写更关键的问题是不建立跨线程可见性与有序性；需要发布关系时依然要使用 `volatile`、锁或其他同步机制。
